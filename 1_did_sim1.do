...
twoway(line y_mean post if treat == 1, color(stc1))(scatter y_mean post if treat == 1, color(stc1) msize(large)) ///
      (line y_mean post if treat == 0, color(stc2))(scatter y_mean post if treat == 0, color(stc2) msize(large)) ///
	  , xlabel(minmax, nogrid) ylabel(, nogrid) legend(order(2 "Treatment" 4 "Control"))
